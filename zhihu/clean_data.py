...
def save_matched_info_from_meta_data(meta_file, review_file):
    # 读取CSV文件
    meta_df = pd.read_csv(meta_file, encoding='utf-8', dtype={'article_id': str, 'question_id': str, 'answer_id': str, 'created_time': str})
    review_df = pd.read_csv(review_file, encoding='utf-8', dtype={'article_id': str, 'question_id': str, 'answer_id': str, 'comment_id': str, 'super_comment_id': str})
    ...
    if 'comment_id' not in review_df.columns or 'super_comment_id' not in review_df.columns:
    ...
    # 匹配到的行
    matched_meta = meta_df[meta_df['article_id'].astype(str).isin(review_article_ids) |
...
def save_matched_info_from_review_data(review_file, meta_file):
    ...
    # 匹配到的行
    matched_review = review_df[review_df['article_id'].astype(str).isin(meta_article_ids) | (
                               review_df['question_id'].astype(str).isin(meta_question_ids) & review_df['answer_id'].astype(str).isin(meta_answer_ids))]
    # 保存匹配到的行到新的CSV文件
...
def clean_zhihu_comments_data(input_file, output_file):
    ...
    # 读取CSV文件
    try:
        df = pd.read_csv(input_file, encoding="utf-8-sig", dtype={'article_id': str, 'question_id': str, 'answer_id': str, 'comment_id': str, 'super_comment_id': str})
    except:
        try:
            df = pd.read_csv(input_file, encoding="gbk", dtype={'article_id': str, 'question_id': str, 'answer_id': str, 'comment_id': str, 'super_comment_id': str})
        except:
            df = pd.read_csv(input_file, encoding="utf-8", dtype={'article_id': str, 'question_id': str, 'answer_id': str, 'comment_id': str, 'super_comment_id': str})
    ...
    df = df[(df['content'].str.len() >= 3) | (df['img_url'] != '')]
    ...
    # 统计不同类型的记录数
    text_only = len(df[(df['content'].str.len() >= 3) & (df['img_url'] == '')])
    img_only = len(df[(df['content'].str.len() < 3) & (df['img_url'] != '')])
    text_and_img = len(df[(df['content'].str.len() >= 3) & (df['img_url'] != '')])
...
if __name__ == "__main__":
    ...
    # 保存匹配信息
    save_matched_info_from_meta_data('zhihu/zhihu_results_final/cleaned_zhihu_meta_data.csv', 'zhihu/zhihu_results_final/cleaned_zhihu_comments_data.csv')
    save_matched_info_from_review_data('zhihu/zhihu_results_final/cleaned_zhihu_comments_data.csv', 'zhihu/zhihu_results_final/cleaned_zhihu_meta_data_matched.csv')
